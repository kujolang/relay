# Kujo Relay next-session enhancement backlog — review 25

This is the handoff from the twenty-fifth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review extends checkpoint integrity to
confirmed worktree cleanup so terminal state cannot redirect destructive Git
operations.

## Completed in this review

- [x] Revalidate mission-policy digest before confirmed cleanup.
- [x] Revalidate run identity, source repository, run-owned worktree path, and
  Git metadata before `git worktree remove --force`.
- [x] Revalidate event-chain and receipt-sequence integrity before cleanup.
- [x] Return structured `state_integrity_failure` before destructive cleanup.
- [x] Preserve the existing tampered-worktree-path smoke and add cleanup
  contract coverage.
- [x] Update ADR-041, README, command reference, integration matrix,
...
- [ ] Add authenticated route discovery, certificate policy, and mTLS policy
  for Watchdog endpoints while retaining fail-closed route, secret-safe
  diagnostics, checkpoint, and cleanup integrity as local baselines.
...
- [ ] Add deterministic doctor checks for upstream versions, Watchdog
  ...
  and cleanup migration posture, and release-policy posture.
- [ ] Promote route, checkpoint, and cleanup validation into authenticated
  service and workcell contracts with tenant-scoped egress policy and durable
  receipts.
...
- [ ] Add committed fixture transcripts, JSON Schemas for mission/run/event/
  ...
  contracts, plus a generated architecture diagram backed by real artifacts.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  truncation rejection, verified export, correlated event context, exact Git
  policy rejection, lock stress, live event watch, duration metrics, artifact
  size inventory, Agents SDK tool approval, and Watchdog correlation.
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  ...
  doctor-redaction, resume-integrity, and cleanup-integrity evidence have
...
cleanup migration, and release gates remain unproven.
