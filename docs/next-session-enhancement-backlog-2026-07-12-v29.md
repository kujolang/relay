# Kujo Relay next-session enhancement backlog — review 29

This is the handoff from the twenty-ninth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not an enterprise-production
platform. This review closes telemetry and correlation-input gaps: AI telemetry
now emits Watchdog route posture without copying the configured URL into run
artifacts, and correlation IDs are constrained before transport.

## Completed in this review

- [x] Remove raw `RELAY_WATCHDOG_URL` values from AI telemetry.
- [x] Emit only configured, valid, scheme, and reason route posture fields in
  `relay_telemetry.watchdog_route`, matching `doctor --json`.
- [x] Add regression coverage for remote route and credential-bearing route
  non-disclosure, including the fixture path that previously bypassed live
  route validation.
- [x] Constrain correlation IDs before headers, telemetry, and Watchdog query
  construction; reject query delimiters and replace invalid values.
- [x] Make concurrent store symlink probes race-safe when `.index.lock`
  disappears during a bounded multi-process rebuild.
- [x] Document ADR-045, the telemetry contract, implementation status, and
  the next-session backlog.
...
- [ ] Promote local state-root, evidence, telemetry non-disclosure, and
  correlation-input checks
  into authenticated service, workcell, and no-follow durable-store contracts.

## P2 — presentation and adoption

- [ ] Add committed fixture transcripts, JSON Schemas for mission/run/event/
  receipt/report/doctor/probe/tool-result/store/telemetry contracts, and a
  generated architecture diagram backed by artifacts.
...
- [ ] Add a truthful showcase gallery covering route telemetry non-disclosure,
  state-store symlink rejection, evidence persistence failure, resume/control/
  cleanup integrity, PackWrite atomic generation, Watchdog correlation, and
  the remaining proven smokes.
...
- [ ] State-store, evidence, route, telemetry, input, resume, control,
  cleanup, bridge, fallback, PackWrite, and Watchdog evidence remains
