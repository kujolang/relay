# Kujo Relay Next-Session Enhancement Backlog — 2026-07-13 v67

This backlog records the next enterprise-readiness review after the v67 local
resource and evidence hardening pass. Relay remains a hardened local
...
- Added a pre-bridge 112 KiB serialized AI request ceiling below the 128 KiB
  environment transport limit.
- Added a 1 MiB serialized provider-response ceiling in the AI bridge.
- Added 64 KiB per-call provider-tool argument bounds and duplicate/control-
  character provider call-ID rejection for unambiguous tool-result correlation.
- Denied HTTP proxy environment overrides at the shared child-process boundary
  so provider and tool traffic cannot be redirected through caller-selected
  proxy variables.
- Added 8 MiB write ceilings for event JSONL and receipt JSON evidence, making
  oversized evidence fail closed before a run can appear complete.
- Required PackWrite to produce a safe regular `agent/MASTER.md` artifact,
  rather than treating process exit success as packet proof.
- Added contract coverage for all new size, correlation, environment, and
  bounded-evidence behavior.
- Updated README, command reference, ADRs, integration matrix, implementation
  plan, enterprise review, final engineering report, and this backlog.
