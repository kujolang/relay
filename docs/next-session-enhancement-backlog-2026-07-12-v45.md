# Kujo Relay next-session enhancement backlog — v45
...
extends the shared fail-closed filesystem boundary from the evidence store to
doctor dependency targets and the trusted Agents SDK worker root/source.

## Completed in this review

- [x] Reuse `path_has_symlink_component` in doctor path/dependency checks.
- [x] Reject symlinked parent components before trusted Agents SDK worker use.
- [x] Add dependency-parent redirection contract coverage.
- [x] Extend the dedicated symlink smoke to dependency paths.
- [x] Update the ADR, command reference, integration matrix, implementation
  plan, enterprise review, final report, README, and this backlog.

## P0 — prove external composition and authority

- [ ] Produce trustworthy full Loop Engineering evidence on a stable or
  isolated host, including deterministic cancellation/timeout process
  ownership.
