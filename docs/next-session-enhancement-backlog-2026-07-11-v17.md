# Kujo Relay next-session enhancement backlog — review 17

This is the handoff from the seventeenth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review bounds mission input before JSON
parsing and persistence, preventing oversized or symlinked documents from
amplifying memory, prompt, state, or filesystem risk.

## Completed in this review

- [x] Require mission input paths to exist and be regular files.
- [x] Reject symbolic-linked mission input files.
- [x] Reject mission documents larger than 1 MiB before parsing.
- [x] Add oversized and symlinked mission-spec regression coverage.
- [x] Add the spec-safety Loop Engineering gate.
- [x] Update ADR-032, README, command reference, integration matrix,
...
- [ ] Promote local watch, duration, size, cancellation, symlink-safety, and
  mission-input evidence into durable event/store concurrency gates when
  stronger persistence and sink owners are selected.
...
- [ ] Live, configured-live, fixture, and blocked evidence are separated in
  reports and docs.
- [ ] Concurrent index, live event, duration, size, cancellation, symlink, and
  mission-input evidence have deterministic coverage or the store/metrics
  sinks have moved to stronger durable owners.
