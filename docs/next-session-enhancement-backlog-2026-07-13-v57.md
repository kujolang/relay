# Kujo Relay next-session enhancement backlog — v57

Date: 2026-07-13

This backlog is the handoff from the fifty-seventh enterprise-readiness review.
Relay remains a hardened local alpha/showcase, not universal enterprise
production. The Agents SDK bridge now has a stronger local disclosure boundary;
external authority and provider-integration P0s remain open.

## Delivered in v57

- Added recursive redaction for Agents SDK bridge model output, nested tool
  output, and worker error text.
- Added a credential-shaped output regression probe to the Agents SDK smoke.
- Updated README, command reference, ADR-078, implementation plan, final
