# Kujo Relay next-session enhancement backlog — review 18

This is the handoff from the eighteenth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review hardens persisted JSON parsing and
prevents non-retryable provider failures from triggering duplicate fallback
calls.

## Completed in this review

- [x] Add bounded pre-parse JSON reads for the run index, lock owner, run state,
  receipts, evaluations, changes, reports, and cancellation requests.
- [x] Reject an oversized run index before parsing and rebuild from authoritative
  per-run state.
- [x] Restrict `RELAY_FALLBACK_MODEL` to explicit transient or model-capability
  failure classes.
- [x] Record skipped fallback reasons in `relay_telemetry`.
- [x] Add contract coverage for bounded JSON reads and retryable versus
  non-retryable fallback classification.
- [x] Extend the store smoke with an oversized-index recovery case.
- [x] Update ADR-033/034, README, command reference, integration matrix,
...
- [ ] Promote bounded JSON parsing and fallback classification into durable
  store/provider concurrency gates after stronger storage and telemetry owners
  are selected.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  oversized-index recovery, non-retryable fallback suppression, truncation
  rejection, verified export, correlated event context, exact Git policy
  rejection, lock stress, live event watch, duration metrics, artifact size
  inventory, Agents SDK tool approval, and Watchdog correlation.
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  mission-input, bounded-JSON, and fallback evidence have deterministic
  coverage or their owners have moved to stronger durable contracts.
...
Runtime implementation remains under `src/`, tests under `tests/`, examples
under `examples/`, and operator documentation under `docs/`.
