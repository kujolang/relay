...
| Model budget enforcement | AI SDK + Relay runtime | aggregate mission `max_tokens` is positive and capped at 65,536; each provider request remains capped at 16,384, follow-ups receive the remaining aggregate budget, and negative provider usage cannot reduce accounting | provider-side billing reconciliation and context-aware cost routing remain open | contract token-boundary checks, low-budget fixture mission, provider-tool acceptance | high |
