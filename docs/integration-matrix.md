...
| Run verification | Relay CLI + existing evidence contracts | `runs verify` aggregates authoritative state, complete events, persisted receipts, ChangeBucket, Eval, and report shape checks into `relay-run-verification-v1`; individual artifact readers and export fail closed | signed manifests, upstream correlation IDs, durable storage, partial-export semantics, and repair remain open | store smoke positive verdict plus missing changes/evaluations/export-report failures | medium |
