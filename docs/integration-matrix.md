...
| Run verification | Relay CLI + existing evidence contracts | `runs verify` aggregates authoritative state, complete events, persisted receipts, ChangeBucket, and Eval shape checks into `relay-run-verification-v1`; individual artifact readers fail closed | signed manifests, upstream correlation IDs, durable storage, and repair remain open | store smoke positive verdict plus missing changes/evaluations failures | medium |
