...
| AI telemetry | Watchdog | configured proxy URL | startup/health adapter pending | route metadata; live Watchdog pending | high |
| Mission packet | PackWrite | fake-response `init`, validate generated pack | packet hash/revision metadata pending | 13-file validated pack | medium |
...
| Authority | Agents SDK approvals + MCP/Fence patterns | Relay policy for declarative actions | external approvals pending | denied write/command tests | high |
...
| Workspace isolation | Git worktree/workcell conventions | user-provided isolated repo | automated worktree creation pending | `/tmp` Git fixture | high |
