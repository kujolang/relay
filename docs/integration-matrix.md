...
| Tools | Agents SDK registry/MCP patterns | `src/agent_bridge.kujo` registers `relay.write_file` and `relay.run_command`; worker delegates to `execute_tool_request` | broader tool catalog, model-driven tool loops, tool-result artifact persistence, and authenticated service mode pending | isolated mission tool smoke with approval denial | high |
