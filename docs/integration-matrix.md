...
| Execution evidence | RunLedger | `start`/`finish` subprocess calls plus AgentEvent-compatible JSONL | event metadata now carries workflow, model, provider, packet revision, and RunLedger ID; typed tool/artifact/evaluation/retry receipts remain pending | pass receipt with git commit, correlated event context, store recovery, truncation/tamper, event-integrity, and export smokes | low |
...
| Authority | Agents SDK approvals + MCP/Fence patterns | Agents SDK approval policy/provider wraps Relay's policy worker; Relay remains authoritative for workspace and command checks | interactive approvals, identity, cancellation, and network controls pending | denied Tool Registry write plus direct policy tests | high |
...
| Resource and cache bounds | Kujo runtime + Relay store | mission budgets, bounded process output, fixed subprocess PATH, bounded Agents SDK tool calls, atomic state, atomic index lock, rebuildable index | database-backed retention, crash recovery, and multi-host concurrency pending | output-budget, store/export, tool-budget, and index-lock smokes | medium |
