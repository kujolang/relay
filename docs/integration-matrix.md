...
| Execution evidence | RunLedger | `start`/`finish` subprocess calls plus AgentEvent-compatible JSONL | attach all event IDs in notes pending; local event hashes, chain/state completeness verification, verified export, and locked rebuildable index improve integrity | pass receipt with git commit, store recovery, truncation/tamper, event-integrity, and export smokes | low |
