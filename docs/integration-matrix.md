...
| Capability registry maintenance | Relay common + doctor | bounded posture scan counts records, stale entries, invalid entries, and explicit cleanup results; `doctor --repair` removes only expired or exhausted records while default doctor remains read-only | authenticated multi-host cleanup, crash reconciliation, and retention policy remain open | CLI smoke creates an expiring record, verifies read-only stale reporting, repairs it, and verifies a clean follow-up scan | high |
