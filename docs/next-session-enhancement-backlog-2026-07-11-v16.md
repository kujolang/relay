# Kujo Relay next-session enhancement backlog — review 16

This is the handoff from the sixteenth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review closes a local evidence-redirection
gap by refusing symbolic-linked and non-regular JSON/JSONL artifacts.

## Completed in this review

- [x] Reject symbolic-linked and non-regular files in common JSON reads.
- [x] Reject symbolic-linked and non-regular files before JSONL append.
- [x] Make `runs watch`, `runs events`, and `runs export` fail closed for unsafe
  `events.jsonl` paths.
- [x] Extend store smoke coverage with an `events.jsonl` symlink to
  `/etc/passwd` and verify the read boundary rejects it.
- [x] Update ADR-031, README, command reference, integration matrix,
...
- [ ] Promote local watch, duration, size, cancellation, and symlink-safety
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  event/receipt integrity, symlink rejection, truncation rejection, verified
...
- [ ] Live, configured-live, fixture, and blocked evidence are separated in
  reports and docs.
- [ ] Concurrent index, live event, duration, size, cancellation, and
  symlink-safety evidence have deterministic coverage or the store/metrics
