# Kujo Relay next-session enhancement backlog — review 35

This is the handoff from the thirty-fifth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not an enterprise-production
platform. This review adds independent executable evidence that command timeout
termination handles descendants and produces a typed timeout result.

## Completed in this review

- [x] Add `tests/relay_timeout_smoke.sh` with a 30-second descendant command and
  one-second timeout.
- [x] Prove return within 12 seconds, no orphaned slow process, and persisted
  `timed_out: true` / `failure_class: timeout` evidence.
- [x] Add the timeout smoke to the local Loop Engineering acceptance set.
...
- [ ] Promote local redaction, dependency, evidence, route, telemetry,
  correlation, diagnostic, failure-class, cancellation, and timeout checks
  into authenticated service and workcell contracts.
...
- [ ] Add a truthful showcase gallery covering dependency-integrity doctor
  ...
  independent descendant timeout proof, process groups, Watchdog diagnostic
  and telemetry non-disclosure, correlation replacement, store-lock race
  safety, state-store symlink rejection, evidence persistence failure,
...
- [ ] Redaction, dependency, state-store, evidence, route, telemetry,
  ...
  fallback, PackWrite, Watchdog, cancellation, and timeout evidence remains
...
bash tests/relay_timeout_smoke.sh
git diff --check
```
