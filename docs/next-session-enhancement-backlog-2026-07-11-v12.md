...
enterprise-production platform. This review adds a bounded live event watcher
over the existing verified JSONL evidence stream, without introducing a
daemon, second event store, or remote subscription claim.

## Completed in this review

- [x] Add `runs watch <run-id>` for machine-readable live AgentEvent JSONL
  observation while a mission is active.
- [x] Bound watcher polling to 1–1000 ms and total wait time to 1–600000 ms.
- [x] Verify the event hash/parent chain on every poll and emit each complete
  event exactly once.
- [x] Tolerate the short final state/file persistence race, then require
  terminal event-sequence and receipt consistency before success.
- [x] Fail closed on malformed, oversized, or terminally inconsistent evidence.
- [x] Add a concurrent mission watch smoke and include it in Loop Engineering
  gates.
- [x] Update ADR-027, README, command reference, integration matrix,
...
- [ ] Promote the local watcher and lock stress harness into durable event/store
  concurrency gates when stronger persistence and sink owners are selected.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  exact Git policy rejection, lock stress, live event watch, Agents SDK tool
...
- [ ] Live, configured-live, fixture, and blocked evidence are separated in
  reports and docs.
- [ ] Concurrent index and live event behavior has deterministic stress evidence
  or the store/event sink has moved to stronger durable owners.
