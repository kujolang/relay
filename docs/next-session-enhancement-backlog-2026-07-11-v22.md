# Kujo Relay next-session enhancement backlog — review 22

This is the handoff from the twenty-second enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review requires HTTPS for non-loopback
Watchdog routes while preserving HTTP for local loopback development.
...
- [x] Allow cleartext HTTP only for localhost, 127.0.0.1, and [::1].
- [x] Require HTTPS for non-loopback Watchdog hosts.
- [x] Return structured invalid_watchdog_route failures.
- [x] Add contract coverage for loopback HTTP, external HTTPS, external HTTP
  rejection, and IPv6 loopback.
- [x] Add CLI coverage for external HTTP rejection.
- [x] Update ADR-038, README, command reference, integration matrix,
...
- [ ] Add authenticated route discovery, certificate policy, and mTLS policy
  for Watchdog endpoints while retaining the current fail-closed URL validation
  and loopback HTTP allowance as the local baseline.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  acceptance, remote HTTP rejection, truncation rejection, verified export,
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  mission-input, bounded-JSON, fallback, worker-root, bridge-payload, and route
  safety evidence have deterministic coverage or their owners have moved to
  stronger contracts.
