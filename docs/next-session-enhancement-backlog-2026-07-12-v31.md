# Kujo Relay next-session enhancement backlog — review 31

This is the handoff from the thirty-first enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not an enterprise-production
platform. This review closes the active-command cancellation gap by composing
Relay's run-owned cancellation marker with Kujo's Unix process-group runtime.

## Completed in this review

- [x] Pass the run-owned `cancel.request.json` to Kujo's `spawn_process`
  `cancel_file` option for active mission commands.
- [x] Terminate Unix descendant processes through Kujo process groups on
  cancellation and timeout (`f24b3c3`).
- [x] Add contract coverage for the cancellation option and a 30-second
  descendant cancellation smoke with an eight-second return bound.
- [x] Document the Relay/Kujo ownership boundary and non-Unix fallback.
...
- [ ] Extend cancellation from process termination to rollback-aware workcell
  semantics and explicit process ownership/retention.
- [ ] Define and test the non-Unix process-tree cancellation contract.
...
- [ ] Add a truthful showcase gallery covering cancellation process groups,
...
- [ ] State-store, evidence, route, telemetry, diagnostic, input, resume,
  control, cleanup, bridge, fallback, PackWrite, Watchdog, and cancellation
  evidence remains deterministic or has moved to a stronger owner contract.
