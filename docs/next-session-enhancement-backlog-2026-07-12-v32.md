# Kujo Relay next-session enhancement backlog — review 32

This is the handoff from the thirty-second enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not an enterprise-production
platform. This review expands the local evidence redaction boundary so
structured credentials and common provider token formats are not copied into
run evidence by default.

## Completed in this review

- [x] Redact structured API/access/auth token, client-secret, password, and
  generic secret fields in subprocess and adapter evidence.
- [x] Redact common OpenAI, AWS, GitHub, and Slack token formats plus private
  key markers.
- [x] Add deterministic contract coverage for JSON credentials, provider
  tokens, private-key markers, bearer headers, and environment-style secrets.
...
- [ ] Promote local redaction, evidence, route, telemetry, correlation, and
  diagnostic checks into authenticated service and workcell contracts.
...
- [ ] Add a truthful showcase gallery covering structured evidence redaction,
  cancellation process groups, Watchdog diagnostic and telemetry
  non-disclosure, correlation replacement, store-lock race safety,
  state-store symlink rejection, evidence persistence failure,
  resume/control/cleanup integrity, PackWrite generation, Watchdog
...
- [ ] At least one external provider is exercised through real Watchdog with
  authenticated correlation evidence and redaction proof.
...
- [ ] Redaction, state-store, evidence, route, telemetry, diagnostic, input,
  resume, control, cleanup, bridge, fallback, PackWrite, Watchdog, and
  cancellation evidence remains deterministic or has moved to a stronger
  owner contract.
...
full Redact integration, certificate validation, mTLS, migration, and release
