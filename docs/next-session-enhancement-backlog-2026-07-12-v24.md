# Kujo Relay next-session enhancement backlog — review 24

This is the handoff from the twenty-fourth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review binds paused-run resume to the
mission policy, workspace, evidence, and budget state that was approved before
the checkpoint.

## Completed in this review

- [x] Persist a SHA-256 digest for every mission policy used by a run.
- [x] Verify run identity and mission-policy integrity before resume.
- [x] Verify provided/worktree workspace identity, source repository, Git
  metadata, and run-owned worktree path before resumed execution.
- [x] Verify effective budgets and attempt bounds before resume.
- [x] Verify persisted event-chain and receipt-sequence integrity before resume.
- [x] Return structured `state_integrity_failure` before any resumed action.
- [x] Add contract coverage for matching and substituted resume state.
- [x] Add tampered paused-workspace and mission-policy CLI smoke coverage.
- [x] Remove a duplicate README test entry and document the new evidence.
- [x] Update ADR-040, README, command reference, integration matrix,
...
- [ ] Add authenticated route discovery, certificate policy, and mTLS policy
  ...
  secret-safe diagnostics, and checkpoint integrity as local baselines.
...
- [ ] Add deterministic doctor checks for upstream versions, Watchdog
  ...
  migration posture, and release-policy posture.
- [ ] Promote route and checkpoint validation into authenticated service and
  workcell contracts with tenant-scoped egress policy and durable receipts.

## P2 — presentation and adoption

- [ ] Add committed fixture transcripts, JSON Schemas for mission/run/event/
  receipt/report/doctor/probe/tool-result/sizes/cancellation/resume contracts,
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  non-disclosure, tampered resume rejection, truncation rejection, verified
  export, correlated event context, exact Git policy rejection, lock stress,
  live event watch, duration metrics, artifact size inventory, Agents SDK tool
  approval, and Watchdog correlation.
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  ...
  doctor-redaction, and resume-integrity evidence have deterministic coverage
  or their owners have moved to stronger contracts.
...
certificate validation, mTLS, authenticated route discovery, checkpoint
migration, and release gates remain unproven.
