# Kujo Relay Next-Session Enhancement Backlog — v83

Relay remains a local-first hardened alpha/showcase, not a universally useful
enterprise-production platform. v83 serializes capability revocation with the
existing authority lock and makes the Agents SDK smoke deterministic across
repeated runs. The next session should prioritize external authority,
provider-backed evidence, and ecosystem composition.
...
- [ ] Add committed fixture transcripts, generated architecture diagrams, and
  a truthful showcase gallery backed by machine-readable artifacts.
- [ ] Add repeatable install/upgrade and dependency provenance checks for
  first-time users on supported macOS/Linux/CI paths.
