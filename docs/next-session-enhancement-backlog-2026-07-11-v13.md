...
enterprise-production platform. This review adds bounded elapsed-time evidence
to the existing AI, tool, and ecosystem adapter boundaries without inventing a
second metrics backend.

## Completed in this review

- [x] Record non-negative adapter `duration_ms` measurements for bounded
  subprocess calls.
- [x] Propagate AI bridge duration into `relay_telemetry`.
- [x] Preserve duration evidence in adapter results and repository action
  results where the boundary owns the measurement.
- [x] Add fixture chat/mission metrics coverage and a Loop Engineering gate.
- [x] Document the scope: local elapsed measurements, not billing, queue time,
  provider SLA, or enterprise metrics aggregation.
- [x] Update ADR-028, README, command reference, integration matrix,
...
- [ ] Add aggregate latency, token/cost, retry, queue, tool-duration,
  ...
  contracts, preserving the local duration evidence as the raw measurement.
...
- [ ] Promote local watch, duration, and lock stress evidence into durable
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  exact Git policy rejection, lock stress, live event watch, duration metrics,
...
- [ ] Live, configured-live, fixture, and blocked evidence are separated in
  reports and docs.
- [ ] Concurrent index, live event, and duration evidence have deterministic
  coverage or the store/metrics sinks have moved to stronger durable owners.
