# Kujo Relay Next-Session Enhancement Backlog — v85

Relay remains a hardened local alpha/showcase, not an enterprise-production
platform or universally useful runtime. v85 makes locked run-index rebuilds
report native write, read-back, and cache-validation failures truthfully. The
next session should move toward external authority and provider evidence.
...
- [ ] Prove provider-generated Agents SDK tool planning through Watchdog in an
  isolated mission with typed tool-result artifacts, bounded fallback, and
