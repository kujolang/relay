# Kujo Relay next-session enhancement backlog — review 20

This is the handoff from the twentieth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review adds bounded, structured input
handling to the AI, Agents SDK, and tool-worker environment bridges.

## Completed in this review

- [x] Cap AI bridge environment payloads at 128 KiB before JSON parsing.
- [x] Cap Agents SDK bridge environment payloads at 128 KiB before parsing.
- [x] Cap tool-worker environment requests at 128 KiB before parsing.
- [x] Return structured invalid or non-object payload errors instead of raw
  interpreter failures.
- [x] Add oversized and malformed payload coverage across all three bridges.
- [x] Update ADR-036, README, command reference, integration matrix,
...
- [ ] Add authenticated file/socket transport for prompts and tool plans larger
  than the 128 KiB environment bridge limit.
...
- [ ] Add deterministic doctor checks for upstream versions, Watchdog
  ...
  payload transport, and release-policy posture.
- [ ] Promote bridge input limits into authenticated service and workcell
  contracts with tenant-scoped authority and durable request receipts.
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  mission-input, bounded-JSON, fallback, worker-root, and bridge-payload
  evidence have deterministic coverage or their owners have moved to stronger
  contracts.
...
"$KUJO_BIN" check src/ai_bridge.kujo
"$KUJO_BIN" check src/agent_bridge.kujo
"$KUJO_BIN" check src/cli.kujo
bash tests/relay_input_boundary_smoke.sh
git diff --check
```
