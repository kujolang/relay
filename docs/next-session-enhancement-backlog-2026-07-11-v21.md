# Kujo Relay next-session enhancement backlog — review 21

This is the handoff from the twenty-first enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review validates the live Watchdog URL
before any AI SDK provider subprocess starts.

## Completed in this review

- [x] Require live Watchdog routes to use HTTP or HTTPS.
- [x] Reject embedded Watchdog URL credentials before provider invocation.
- [x] Return structured invalid_watchdog_route failures.
- [x] Add contract and CLI invalid-route regression coverage.
- [x] Update ADR-037, README, command reference, integration matrix,
...
- [ ] Add authenticated route discovery and TLS policy for Watchdog endpoints,
  retaining the current fail-closed URL validation as the local baseline.
...
- [ ] Add deterministic doctor checks for upstream versions, Watchdog
  ...
  payload transport, route TLS posture, and release-policy posture.
- [ ] Promote route validation into authenticated service and workcell
  contracts with tenant-scoped egress policy and durable request receipts.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  invalid Watchdog-route rejection, truncation rejection, verified export,
  correlated event context, exact Git policy rejection, lock stress, live event
  watch, duration metrics, artifact size inventory, Agents SDK tool approval,
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  mission-input, bounded-JSON, fallback, worker-root, bridge-payload, and
  route-safety evidence have deterministic coverage or their owners have moved
  to stronger contracts.
...
workcell recovery, authenticated machine mode, durable multi-host storage, TLS
route policy, and release gates remain unproven.
