# Kujo Relay next-session enhancement backlog — review 26

This is the handoff from the twenty-sixth enterprise-readiness review. Relay
remains a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review routes operator pause and cancel
through the same checkpoint integrity boundary as resume and cleanup, and
restores the PackWrite atomic-write integration needed for truthful mission
evidence.

## Completed in this review

- [x] Reuse mission-policy, workspace, budget, event, and receipt integrity
  checks for non-terminal pause and cancel mutations.
- [x] Return structured `state_integrity_failure` before tampered control
  mutations.
- [x] Strengthen the resume smoke to require a genuinely paused run and verify
  clean cancellation after tamper checks.
- [x] Complete PackWrite’s existing atomic-write migration with a bounded
  sibling-temp writer and import it in the pack writer.
- [x] Verify PackWrite with 144 unit and 47 CLI integration assertions.
- [x] Update ADR-042, README, command reference, integration matrix,
...
- [ ] Add authenticated route discovery, certificate policy, and mTLS policy
  ...
  checkpoint, cleanup, pause, and cancel integrity as local baselines.
...
- [ ] Add deterministic doctor checks for upstream versions, Watchdog
  ...
  cleanup, pause/cancel migration posture, and release-policy posture.
- [ ] Promote route, checkpoint, cleanup, and control validation into
...
- [ ] Add committed fixture transcripts, JSON Schemas for mission/run/event/
  ...
  control contracts, plus a generated architecture diagram backed by artifacts.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  non-disclosure, tampered resume rejection, tampered cleanup rejection,
  tampered pause/cancel rejection, truncation rejection, verified export,
  correlated event context, exact Git policy rejection, lock stress, live
  event watch, duration metrics, artifact size inventory, Agents SDK tool
  approval, PackWrite atomic generation, and Watchdog correlation.
...
- [ ] Concurrent index, live event, duration, size, cancellation, symlink,
  ...
  doctor-redaction, resume, cleanup, pause/cancel, and PackWrite evidence have
...
certificate validation, mTLS, authenticated route discovery, checkpoint and
control migration, and release gates remain unproven.
