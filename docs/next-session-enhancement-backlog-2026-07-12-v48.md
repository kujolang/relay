# Kujo Relay next-session enhancement backlog — v48
...
closed an evidence-consistency gap: authoritative state and the verified JSONL
event log must now contain the same complete records, not merely matching IDs.

## Completed in this review

- [x] Compare complete canonical event records between `state.events` and
  `events.jsonl`, including payload and metadata.
- [x] Add contract coverage for state-only event payload divergence.
- [x] Add store-smoke coverage proving `runs events` fails closed on state/log
  divergence.
- [x] Preserve the v47 sibling-path/state-store policy and module-context fixes.
- [x] Update ADR-066, README, command/reference docs, integration matrix,
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  integrity, state/log divergence rejection, truncation rejection, verified
  export, Agents SDK tool approval, dangling-link rejection, ChangeBucket
  evidence, and Watchdog correlation.
