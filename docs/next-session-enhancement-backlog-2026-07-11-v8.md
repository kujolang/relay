# Kujo Relay next-session enhancement backlog — review 8

This is the handoff from the eighth enterprise-readiness review. Relay remains
a local-first hardened alpha/showcase, not a universally useful,
enterprise-production platform. This review closes a local evidence-integrity
gap: a valid event prefix can no longer be mistaken for a complete run.

## Completed in this review

- [x] Compare parsed event IDs and order with authoritative `state.events`.
- [x] Reject truncated event logs even when the remaining prefix hashes
  correctly.
- [x] Reject malformed event records and oversized event logs before inspection
  or export; inspection is bounded to 8 MiB.
- [x] Preserve versioned `runs export` as a verified machine boundary.
- [x] Add contract coverage for sequence truncation and store-smoke coverage
  for both truncation and payload tampering.
- [x] Re-audit root layout: tracked root files remain only the necessary Kujo
  entrypoint, package manifest, README, and thin launcher; runtime stays under
  `src/`.
...
- [ ] Add a truthful showcase gallery covering chat, probe, bounded task,
  ...
  truncation rejection, verified export, Agents SDK tool approval, and Watchdog
  correlation.
