...
grep -q 'capability_already_registered' <<<"$duplicate_capability"
...
grep -q 'state_store_failure' <<<"$root_output"
grep -q 'unsafe' <<<"$root_output"
...
grep -q 'state_store_failure' <<<"$root_mission_output"
test ! -e "$TARGET/runs"
...
grep -q 'PASS parent symlink store rejected' <<<"$parent_probe_output"
...
grep -q 'state_store_failure' <<<"$runs_output"
...
grep -q 'state_store_failure' <<<"$runs_mission_output"
test ! -e "$TARGET/runs"
