...
version="$(tr -d '\r\n' < "$ROOT/VERSION")"
test "$version" = "1.1.0"
grep -Fqx 'version = "1.1.0"' "$ROOT/kujo.toml"
grep -Fqx 'version = "1.1.0"' "$ROOT/kennel.toml"
grep -Fq 'export RELAY_VERSION := "1.1.0"' "$ROOT/src/contracts.kujo"
grep -Fq 'export MISSION_SPEC_VERSION := "1.0.0"' "$ROOT/src/contracts.kujo"
grep -Fq '[![Version](https://img.shields.io/badge/version-1.1.0-black)]' "$ROOT/README.md"
grep -Fq '## [1.1.0]' "$ROOT/CHANGELOG.md"
...
if [[ -x "$KUJO" ]]; then
  test "$(KUJO_BIN="$KUJO" "$ROOT/bin/relay" --version)" = "relay 1.1.0"
fi
